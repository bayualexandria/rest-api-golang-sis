...
func SendNewPassword(to string, data interface{}) {

	mailer := config.EmailConfig()
	// Load HTML template
	body, err := template.LoadHTMLTemplate("./templates/mail/new_password.html", data)
	if err != nil {
		return
	}
	...
	mail.SetHeader("Subject", "Change New Password")
